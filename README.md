...
This script will also apply settings found in the module `dash_practice.prod_settings` (or a custom Python file supplied
with the `--settings` flag) and which takes precedence over the same settings
found in `dash_practice.settings`.
...
`run-dash_practice-prod` script to take effect even if you
...
for example) with the `dash_practice.wsgi` entry point
